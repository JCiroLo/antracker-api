...
async function migrate() {
    ...
    const { data: incomeTemplates } = await supabase_1.default.from("income_templates").select("*");
    ...
    const { data: expenseRecords } = await supabase_1.default.from("expense_records").select("*");
    const { data: incomeRecords } = await supabase_1.default.from("income_records").select("*");
    ...
    for (let i = 0; i < recordsToInsert.length; i += chunkSize) {
        ...
        await supabase_1.default.from("transaction_records").insert(chunk);
    }
